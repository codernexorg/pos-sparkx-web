...
import {createUser, fetchUsers} from "../../../redux/actions/user";
...
const User: React.FC<UserProps> = () => {
    ...
    return (
        <div className={'mt-10'}>
            <Modal footer={false} open={showModal} onCancel={() => {
                setShowModal(false)
                setUserToEdit(null)
            }
            }>
                ...
                }}>
                    {
                        ({handleSubmit}) => (
                            <Form method={'POST'} onSubmit={(e) => {
                                ...
                                <CommonInput label={"Email"} name={'email'} placeholder={'Email'}/>
                                <CommonInput label={"Username"} name={'username'} placeholder={'Username'}/>
                                <CommonInput label={"Password"} name={'password'} placeholder={'Password'}/>
                                ...
                                <SelectInput label={"Assign Showroom"} name={'assignedShowroom'}>
                                    <option value={"All"}>All</option>
                                    {
                                        showroom.map((showroom) => (
                                            <option key={showroom.id}
                                                    value={showroom.showroomName}>{showroom.showroomName}</option>
                                        ))
                                    }
                                </SelectInput>
            ...
            <Table dataSource={
                user
            } loading={isLoading} rowKey={(obj: IUser, index) => obj.username + index}>
                <Table.Column title={'#'} render={(text, record, index) => index + 1}/>
                <Table.Column title={"Name"} dataIndex={'name'}/>
                <Table.Column title={"Role"} dataIndex={'role'}/>
                <Table.Column title={'Showroom Assigned'} dataIndex={`assignedShowroom`}
                              render={(_, record: IUser) => {
                                  return record.assignedShowroom
                              }}/>
                <Table.Column title={"Actions"} render={(_, record: IUser) => {
                    return (
                        <div>
                            <AntButton onClick={() => {
                            ...
                            }><AiOutlineEdit/></AntButton>
                            <AntButton><AiOutlineDelete/></AntButton>
                        </div>
                    )
                }}/>
            </Table>
